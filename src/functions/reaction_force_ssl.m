function [Fx1,Fy1,Fz1,Mx1,My1,Mz1,Fx2,Fy2,Fz2,Mx2,My2,Mz2] = reaction_force_ssl(body_xi,body_yi,body_zi,joint_poses,joint_rot_matrices,body_joints,l_list,xg_list,xxi,yyi,zzi,mi,tss)
    ...
    %% Right foot (flying) to hip
    % Right foot to right lower leg
    ...
    % Forces to provide to the point "Ar" of the hip
    XAr = -F_new(1,:);
    YAr = -F_new(2,:);
    ZAr = -F_new(3,:);
    LAr = -M_new(1,:);
    ...
    %% Hip to left upper leg
    body = find(body_ids == "hip");
    [dxB, dyB, dzB, dxG, dyG, dzG, ddxG, ddyG, ddzG, ddyB, ddzB, theta, phi, omega_x, omega_y, omega_z, domega_x, domega_y, domega_z] = body_motion(joint_poses,joint_rot_matrices,body,body_joints,l_list,xg_list,tss,body_xi,body_yi,body_zi);
    [XAl,YAl,ZAl,LAl,MAl,NAl] = hip_dyna_ssl(body,XAr,YAr,ZAr,LAr,MAr,NAr,XA_hip,YA_hip,ZA_hip,LA_hip,MA_hip,NA_hip,dxB, dyB, dzB, dxG, dyG, dzG, ddxG, ddyG, ddzG, ddyB, ddzB, theta, phi, omega_x, omega_y, omega_z, domega_x, domega_y, domega_z,xxi,yyi,zzi,mi,l_list,xg_list,N,joint_poses,body_xi,body_yi,body_zi);
    % Rotate the forces to give them to the next body
    F_new = zeros(3,N);
    M_new = zeros(3,N);
    for timestep=1:N
        F = [XAl(timestep) YAl(timestep) ZAl(timestep)]';
        M = [LAl(timestep) MAl(timestep) NAl(timestep)]';
        body = find(body_ids == "hip");
        R0 = get_R(body,timestep,body_xi,body_yi,body_zi);
    ...
    % Forces to provide to the point "A" of the left upper leg
    XAl = -F_new(1,:);
    YAl = -F_new(2,:);
    ZAl = -F_new(3,:);
    LAl = -M_new(1,:);
    ...
    %% Left upper leg to left lower leg
    body = find(body_ids == "left upper leg");
    [dxB, dyB, dzB, dxG, dyG, dzG, ddxG, ddyG, ddzG, ddyB, ddzB, theta, phi, omega_x, omega_y, omega_z, domega_x, domega_y, domega_z] = body_motion(joint_poses,joint_rot_matrices,body,body_joints,l_list,xg_list,tss,body_xi,body_yi,body_zi);   
    [XB,YB,ZB,LB,MB,NB] = two_wrench_dyna(body,XAl,YAl,ZAl,LAl,MAl,NAl,dxB, dyB, dzB, dxG, dyG, dzG, ddxG, ddyG, ddzG, ddyB, ddzB, theta, phi, omega_x, omega_y, omega_z, domega_x, domega_y, domega_z,xxi,yyi,zzi,mi,l_list,xg_list,N);
    % Rotate the forces to give them to the next body
    F_new = zeros(3,N);
    M_new = zeros(3,N);
    for timestep=1:N
    ...
    % Forces to provide to the point "A" of the left lower leg
    XAl = -F_new(1,:);
    YAl = -F_new(2,:);
    ZAl = -F_new(3,:);
    LAl = -M_new(1,:);
    ...
    % Forces to provide to the point "A" of the left foot
    XAl = -F_new(1,:);
    YAl = -F_new(2,:);
    ZAl = -F_new(3,:);
    LAl = -M_new(1,:);
    ...
    % Forces to provide to the left force plate
    Fx2 = F_new(1,:);
    Fy2 = F_new(2,:);
    Fz2 = F_new(3,:);
    Mx2 = M_new(1,:);
